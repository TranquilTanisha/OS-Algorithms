def c_scan():
    ...
    if dir=="higher":
        ...
        total_seek+=abs(t2[len(t2)-1]-r[1])
        total_seek+=abs(r[1]-r[0])
        # t1.reverse()
        
        for i in range(len(t1)):
            if i==0:
                total_seek+=abs(r[0]-t1[i])
            else:
        ...
        for i in range(len(t1)):
            if i==0:
                ...
                total_seek+=abs(t1[i]-t1[i-1])
        total_seek+=abs(t1[len(t1)-1]-r[0])
        total_seek+=abs(r[0]-r[1])
        t2.reverse()

        for i in range(len(t2)):
            if i==0:
                total_seek+=abs(r[1]-t2[i])
            else:
                total_seek+=abs(t2[i]-t2[i-1])
